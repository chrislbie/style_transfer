...
try:
    from models.modules import Transpose2dBlock, Conv2dBlock, ExtraConvBlock, Conv2dBlock2
...
class Content_Encoder(nn.Module):
    def __init__(self,
        ...
        for i in range(len(channel_numbers)-1):
            ...
            # add convolution
            layers.append(Conv2dBlock2(in_ch, out_ch, block_activation, batch_norm, drop_rate, bias, pooling=pooling, num_extra_conv=num_extra_conv, residual=residual))
...
class Style_Transfer_Model_edflow(Style_Transfer_Model):
    def __init__(self, config):
        ...
        residual = config["model_config"]["residual"]
        pooling = None if config["model_config"]["pooling"] == "None" else config["model_config"]["pooling"]
