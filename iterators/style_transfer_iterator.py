...
class Style_Transfer_Iterator(TemplateIterator):
    def __init__(self, config, root, model, *args, **kwargs):
        ...
        self.optimizer_G = torch.optim.Adam(chain(self.model.c_enc.parameters, self.model.s_enc.parameters, self.model.d_enc.parameters), lr=config["optimizer"]["lr"], betas=(.5, .999))
        D_lr_factor = self.config["optimizer"]["D_lr_factor"] if "D_lr_factor" in config["optimization"] else 1
    ...
    def criterion(self, content_images, style_images, output, style_label):
        ...
        fpt1 = nn.MSELoss()(self.model.s[:1], self.model.s_enc(self.model.dec(content_images[:1], self.model.s[:1])))
        fpt2 = nn.MSELoss()(self.model.s[:1], self.model.s_enc(self.model.dec(content_images[:1], self.model.s[:2])))
